...
ll mod = 1e9 + 7;
ll modu = 998244353;
const int N = 1000000;  // Based on max n in calculation of ncr (we can get it from the input range)
...
int main()
{
    ...
    while(t--)
    {
        ...
        for(i=0;i<n;i++)
        {
            ...
            ll pres_right = n-i-1;
            //cout << ans << endl;
            if(req_left==pres_left)
            {
                ans += long_power_long(2,pres_right);
            }
            else
            {
                //cout << i << " " << pres_left << " " << req_left << ncr(pres_left,req_left) << endl;
                if(ncr(pres_left,req_left) >= 1)
                    j = ncr(pres_left,req_left);
                else
                    j=1;
                //cout << "j" << j << endl;
                ans += (j*long_power_long(2,pres_right));
            }
        }
        cout << ans << endl;
