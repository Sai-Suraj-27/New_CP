...
int main()
{
    ...
    while(t--)
    {
        ...
        pos_1 = min_element(all(v)) - v.begin();
        pos_2 = max_element(all(v)) - v.begin();

        cout << min({max(pos_1,pos_2)+1, max(n-pos_1, n-pos_2), min(pos_1,pos_2)+1+min(n-pos_1,n-pos_2)}) << endl;
